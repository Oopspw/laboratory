...
JsEncodePlugin.prototype.apply = function (compiler) {
  ...
  compiler.plugin('after-emit', function (compilation, callback) {
      ...
      var filePath = path.resolve(__dirname, self.options.assetsPath);//设置需要加密的js文件路径，_this.options.assetsPath为插件配置中传过来的需要加密的js文件路径
      filterFile(filePath);
      function filterFile(fp){
          ...
          files.forEach((filename)=>{
						//遍历该路径下所有文件
            if(self.options.jsReg.test(filename)){
							//利用正则匹配我们要加密的文件,self.options.jsReg为插件中传过来的需要加密的js文件正则，用以筛选出我们需要加密的js文件。
              ...
              fs.readFile(filedir, 'utf-8', (err, data)=>{
                ...
                let result = jjencode(self.options.global, data);
